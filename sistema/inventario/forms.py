...
class ProductoFormulario(forms.ModelForm):
    precio = forms.DecimalField(
        ...
        attrs={'placeholder': 'Precio del Insumo',
    ...
    class Meta:
        ...
        widgets = {
        'descripcion': forms.TextInput(attrs={'placeholder': 'Nombre del Insumo',
...
class ClienteFormulario(forms.ModelForm):
    tipoC =  [ ('1','DNI'),('2','CARNÉ DE EXTRANJERÍA') ]

    telefono2 = forms.CharField(
        required = False,
        label = 'Segundo numero telefonico',
        widget = forms.TextInput(
        attrs={'placeholder': 'Inserte el telefono alternativo del transportista',
    ...
    correo2 = forms.CharField(
        ...
        widget = forms.TextInput(
        attrs={'placeholder': 'Inserte el correo alternativo del transportista',
    ...
    class Meta:
        ...
        labels = {
        'cedula': 'Cedula del Transportista',
        'nombre': 'Nombre del Transportista',
        'apellido': 'Apellido del Transportista',
        'direccion': 'Direccion del Transportista',
        'nacimiento': 'Fecha de nacimiento del Transportista',
        'telefono': 'Numero telefonico del Transportista',
        'correo': 'Correo electronico del Transportista',
        'telefono2': 'Segundo numero telefonico',
        'correo2': 'Segundo correo electronico'
        }
        widgets = {
        'cedula': forms.TextInput(attrs={'placeholder': 'Inserte la cedula de identidad del Transportista',
        'id':'cedula','class':'form-control'} ),
        'nombre': forms.TextInput(attrs={'placeholder': 'Inserte el primer o primeros nombres del Transportista',
        'id':'nombre','class':'form-control'}),
        'apellido': forms.TextInput(attrs={'class':'form-control','id':'apellido','placeholder':'El apellido del Transportista'}),
        'direccion': forms.TextInput(attrs={'class':'form-control','id':'direccion','placeholder':'Direccion del Transportista'}), 
        ...
        'placeholder':'El telefono del transportista'} ),
        'correo':forms.TextInput(attrs={'placeholder': 'Correo del Transportista',
...
class EmitirFacturaFormulario(forms.Form):
    def __init__(self, *args, **kwargs):
       ...
       if(elecciones):
            self.fields["cliente"] = forms.CharField(label="Transportista",max_length=50,
            widget=forms.Select(choices=elecciones,
            attrs={'placeholder': 'La cedula del Transportista',
            'id':'cliente','class':'form-control'}))
    
    productos = forms.IntegerField(label="Numero de Insumos",widget=forms.NumberInput(attrs={'placeholder': 'Numero de insumos a Retirar',
        'id':'productos','class':'form-control'}))

class DetallesFacturaFormulario(forms.Form):
    productos = Producto.productosRegistrados()

    descripcion = MisProductos(queryset=productos,widget=forms.Select(attrs={'placeholder': 'El insumo a debitar','class':'form-control select-group','onchange':'establecerOperaciones(this)'}))

    vista_precio = MisPrecios(required=False,queryset=productos,label="Precio del insumo",widget=forms.Select(attrs={'placeholder': 'El precio del insumo','class':'form-control','disabled':'true'}))

    cantidad = forms.IntegerField(label="Cantidad a retirar",min_value=0,widget=forms.NumberInput(attrs={'placeholder': 'Introduzca la cantidad del insumo','class':'form-control','value':'0','onchange':'calculoPrecio(this);calculoDisponible(this)', 'max':'0'}))

    cantidad_disponibles = forms.IntegerField(required=False,label="Stock disponible",min_value=0,widget=forms.NumberInput(attrs={'placeholder': 'Introduzca la cantidad del insumo','class':'form-control','value':'0', 'max':'0', 'disabled':'true'}))

    selec_disponibles = MisDisponibles(queryset=productos,required=False,widget=forms.Select(attrs={'placeholder': 'El insumo a debitar','class':'form-control','disabled':'true','hidden':'true'}))
...
class EmitirPedidoFormulario(forms.Form):
    def __init__(self, *args, **kwargs):
       ...
       if(elecciones):
            ...
            widget=forms.Select(choices=elecciones,attrs={'placeholder': 'La cedula del proveedor que vende el insumo',
            'id':'proveedor','class':'form-control'}))

    productos = forms.IntegerField(label="Numero de Insumos",widget=forms.NumberInput(attrs={'placeholder': 'Numero de insumos a comprar',
        'id':'productos','class':'form-control'}))
...
class DetallesPedidoFormulario(forms.Form):
    ...
    cantidad = forms.IntegerField(label="Cantidad",min_value=0,widget=forms.NumberInput(attrs={'placeholder': 'Introduzca la cantidad del insumo','class':'form-control','value':'0','onchange':'calculoPrecio(this)'}))
